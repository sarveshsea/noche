...
(function() {
  ...
  function uuidv4() {
    ...
    if (c && c.getRandomValues) {
      ...
      for (let i2 = 0; i2 < 16; i2 += 1) bytes[i2] = Math.floor(Math.random() * 256);
    }
    bytes[6] = bytes[6] & 15 | 64;
    bytes[8] = bytes[8] & 63 | 128;
    let out = "";
    for (let i2 = 0; i2 < 16; i2 += 1) {
      out += byteHex(bytes[i2]);
      if (i2 === 3 || i2 === 5 || i2 === 7 || i2 === 9) out += "-";
    }
    return out;
  }
  ...
  function nodeFingerprint(node) {
    ...
    for (let i2 = 0; i2 < raw.length; i2 += 1) {
      hash ^= raw.charCodeAt(i2);
      hash = hash + ((hash << 1) + (hash << 4) + (hash << 7) + (hash << 8) + (hash << 24)) >>> 0;
    }
    return "v1-" + hash.toString(16);
  }
  const EXEC_MAX_LENGTH = 5e4;
  const EXEC_MAX_TOKENS = 1e4;
  const EXEC_DEFAULT_TIMEOUT_MS = 3e3;
  const BANNED_IDENTIFIERS$1 = [
    "closePlugin",
    "removePage",
    "__proto__",
    "__defineGetter__",
  ...
  const PROTECTED_ROOTS$1 = ["figma", "globalThis", "window", "self"];
  function guardExecCode(raw) {
  ...
  function withExecTimeout(promise, ms = EXEC_DEFAULT_TIMEOUT_MS) {
    let timer;
  ...
  function rejected(code, message, detail) {
  ...
  function normalizeCode(raw) {
    ...
    while (i2 < raw.length) {
      ...
      if (ch === "\\" && next === "u") {
        const braceMatch = /^\\u\{([0-9a-fA-F]+)\}/.exec(raw.slice(i2));
        const hexMatch = /^\\u([0-9a-fA-F]{4})/.exec(raw.slice(i2));
        if (braceMatch) {
          const code = parseInt(braceMatch[1], 16);
          if (code < 65536) out += String.fromCharCode(code);
          else out += "_";
          i2 += braceMatch[0].length;
          continue;
        }
        if (hexMatch) {
          out += String.fromCharCode(parseInt(hexMatch[1], 16));
          i2 += hexMatch[0].length;
          continue;
        }
      }
      out += ch;
  ...
  pp$1.regexp_eatUncapturingGroup = function(state2) {
    ...
    if (state2.eat(
      ...
      if (state2.eat(
        ...
        if (this.options.ecmaVersion >= 16) {
          var addModifiers = this.regexp_eatModifiers(state2);
          var hasHyphen = state2.eat(
            45
            /* - */
          );
          if (addModifiers || hasHyphen) {
            for (var i2 = 0; i2 < addModifiers.length; i2++) {
              ...
              if (addModifiers.indexOf(modifier, i2 + 1) > -1) {
                state2.raise("Duplicate regular expression modifiers");
              }
            }
            if (hasHyphen) {
              var removeModifiers = this.regexp_eatModifiers(state2);
              if (!addModifiers && !removeModifiers && state2.current() === 58) {
                state2.raise("Invalid regular expression modifiers");
              }
  ...
  pp$1.regexp_eatModifiers = function(state2) {
    var modifiers = "";
    var ch = 0;
    while ((ch = state2.current()) !== -1 && isRegularExpressionModifier(ch)) {
  ...
  const BANNED_IDENTIFIERS = /* @__PURE__ */ new Set([
    "closePlugin",
    "removePage",
    "__proto__",
    "__defineGetter__",
  ...
  const PROTECTED_ROOTS = /* @__PURE__ */ new Set(["figma", "globalThis", "window", "self"]);
  const AST_MAX_NODES = 2e4;
  function parseAndGuard(source) {
    let ast;
    try {
    ...
    const visit = (node) => {
      ...
      switch (node.type) {
        ...
        case "MemberExpression": {
          const n = node;
          if (n.computed && n.object.type === "Identifier" && PROTECTED_ROOTS.has(n.object.name)) {
            rejection = {
          ...
          if (n.computed && n.property.type === "Literal" && typeof n.property.value === "string" && BANNED_IDENTIFIERS.has(String(n.property.value))) {
            rejection = {
              reason: "Banned computed property: [" + String(n.property.value) + "]",
  ...
  async function executeCode(code) {
    ...
    return await withExecTimeout(fn(figma));
  }
  function getPageTree(maxDepth) {
    function walkChildren(node, depth) {
      if (depth > maxDepth) return null;
      const data2 = { id: node.id, name: node.name, type: node.type, visible: node.visible !== false };
      if ("children" in node && node.children) {
        data2.children = node.children.map((child) => walkChildren(child, depth + 1)).filter(Boolean);
      }
      return data2;
    }
    return {
  ...
  function getFileData(maxDepth) {
    function walk(node, depth) {
      ...
      if ("children" in node && node.children) {
        data2.children = node.children.map((child) => walk(child, depth + 1));
      }
      return data2;
    }
  ...
  async function pushTokens(params) {
    ...
    for (let i2 = 0; i2 < fetched.length; i2 += 1) {
      const entry = fetched[i2];
